...
class LbotRobot:
    ...
    def __init__(self, tcp_host: str = "127.0.0.1"):
        ...
        """
        if api is None:
            raise RuntimeError("LbotAPI is not initialized")
    ...
    def connect(self, timeout: float = 10.0) -> bool:
        ...
        if self._handle:
            ...
            err_msg = "TCP connection failed"
            try:
                handle = self._handle
                if handle is not None and getattr(handle, "handle_id", 0) > 0:
                    err_msg = api.get_last_error()
            except Exception as exc:
                err_msg = str(exc) or err_msg
            print(f"Robot Connect failed: {err_msg}")
            self._handle = None
    ...
    def get_handle(self) -> Optional[LbotHandle]:
        """
        @brief 获取当前句柄
